...
public class OutputProcessor implements LogicalIOProcessor{
	private static final Log LOG=LogFactory.getLog(OutputProcessor.class);
	...
	public void run(Map<String, LogicalInput> inputs, Map<String, LogicalOutput> outputs) throws Exception {
		...
		KeyValueWriter kvWriter = (KeyValueWriter) outputs.values().iterator().next().getWriter();
		for(LogicalInput input:inputs.values()){
